    ...
    return (
        <div className="px-4 py-4 pb-28 space-y-5">
            ...
            <section className="grid gap-4">
                {packages.map((tier) => <TierCard key={tier.id} tier={tier} active={selectedTier === tier.id} onSelect={() => setSelectedTier(tier.id)} />)}
            </section>
            ...
            <section className="rounded-[28px] p-4 space-y-4" style={{ background: 'var(--color-bg-card)', border: 'var(--card-border)', boxShadow: 'var(--shadow-card)' }}>
                <div className="flex items-start justify-between gap-3">
                    <div>
                        <p className="text-xs font-black uppercase text-primary">Selected package</p>
                        <h2 className="text-2xl font-black text-text-primary">{selected.name}</h2>
                        <p className="text-sm font-black text-secondary">KSh {selected.price.toLocaleString()}</p>
                    </div>
                    <SelectedIcon size={28} className={selected.color === 'gold' ? 'text-gold' : selected.color === 'secondary' ? 'text-secondary' : 'text-primary'} />
                </div>
                ...
                <div className="rounded-2xl p-3" style={{ background: 'var(--color-surface)' }}>
                    <p className="mb-2 text-xs font-black text-text-primary">Unlocked services</p>
                    <ol className="space-y-2">
                        {selected.services.map((service, index) => (
                            <li key={service} className="flex gap-3 rounded-xl bg-white/70 p-2">
                                ...
                                <span className="text-sm font-bold leading-snug text-text-secondary">{service}</span>
                            </li>
                        ))}
                    </ol>
                </div>
...
function TierCard({ tier, active, onSelect }) {
    ...
    return (
        <button onClick={onSelect} className="relative overflow-hidden rounded-[28px] p-4 text-left transition-all" style={{ background: 'var(--color-bg-card)', border: tone.border, boxShadow: active ? tone.glow : 'var(--shadow-card)' }}>
            ...
            <div className="mt-4 flex items-center justify-between gap-2">
                ...
                <span className={`rounded-full px-3 py-1 text-xs font-black ${active ? 'gradient-primary text-white' : 'bg-gray-100 text-text-secondary'}`}>{active ? 'Selected' : 'Choose'}</span>
            </div>
        </button>
    );
}
