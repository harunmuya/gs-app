    ...
    return (
        <section className="rounded-2xl p-3" style={{ background: 'var(--color-bg-card)', border: 'var(--card-border)' }}>
            <div className="mb-2 flex items-center justify-between gap-2">
                <h2 className="text-sm font-bold text-text-primary flex items-center gap-1"><Radio size={15} className="text-danger" /> {title}</h2>
                <Link href="/live" className="text-[11px] font-semibold text-primary">Open Live</Link>
            </div>
            {streams.length === 0 ? (
                        ...
                        return (
                            <Link key={stream.id} href={`/live/${stream.id}`} className="min-w-40 rounded-2xl bg-gray-950 p-2 text-white shadow-sm">
                                <div className="relative h-24 overflow-hidden rounded-xl bg-primary/20">
                                    {photo ? <img src={photo} alt="" loading="lazy" decoding="async" className="h-full w-full object-cover" /> : <UserAvatar name={host.display_name || 'Live'} size={52} />}
                                    <span className="absolute left-2 top-2 rounded-full bg-danger px-2 py-0.5 text-[9px] font-semibold">LIVE</span>
                                </div>
                                ...
                                <span className="mt-2 block rounded-xl bg-white/10 py-1.5 text-center text-[10px] font-semibold">Join Live</span>
                            </Link>
                        );
                    })}
                </div>
