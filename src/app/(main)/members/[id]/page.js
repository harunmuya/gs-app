...
function packageAccess(user) {
    ...
    const active = tier !== 'free' && !user?.package_locked;
    return {
        tier,
        canBrowse: true,
        canMessage: true,
...
function legacyMemberKey(searchParams, fallback) {
...
export default function MemberProfilePage({ params }) {
    ...
    const requestedId = id === 'single.php' ? legacyMemberKey(searchParams, id) : id;
    const memberId = member?.id || requestedId;
    const isSelfProfile = member?.id === user?.id;
    const localOnlyMember = isLocalOnlyMember(member, requestedId);
    ...
    useEffect(() => {
        ...
        loadMember();
        return () => { alive = false; };
    }, [requestedId, user?.id, router, locationTick]);
    ...
    useEffect(() => {
        if (!memberId || String(memberId).startsWith('@') || localOnlyMember || !user?.id) return;
        ...
        fetch('/api/members', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'view', memberId, actorKey: user.id || getActorKey(), actorUserId: user.id }) })
            ...
            .catch(() => {});
    }, [memberId, localOnlyMember, user?.id, router]);
    ...
    const memberDistanceText = member.distanceText || profileDistanceText(user, member);
