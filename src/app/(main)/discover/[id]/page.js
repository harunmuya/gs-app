...
export default function ProfileDetailPage({ params }) {
    ...
    const profileId = id === 'single.php' ? legacyPostKey(searchParams, id) : id;
    const {
    ...
    useEffect(() => {
        ...
        loadProfile();
    }, [profileId, logProfileView]);

    // Fetch comments
    useEffect(() => {
        async function loadComments() {
            try {
                const res = await fetch(`/api/comments?post=${encodeURIComponent(profile?.wpId || profileId)}`);
                const data = await res.json();
    ...
    const handleShare = async () => {
        ...
        if (navigator.share) {
            try { await navigator.share({ title: profile.name, text: `Check out ${profile.name} on Genuine Sugar Mummies`, url }); } catch { }
        } else {
