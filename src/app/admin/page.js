        ...
        <div className="admin-dashboard-container flex-1 flex flex-col min-h-screen bg-slate-950 pb-16">
            ...
            <header className="sticky top-0 z-40 bg-slate-900/60 border-b border-slate-800/80 backdrop-blur-xl px-8 py-4 flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button onClick={fetchUsers} className="p-2.5 bg-slate-900 border border-slate-800 hover:border-slate-700 rounded-xl text-slate-400 hover:text-slate-200 transition-all cursor-pointer" title="Refresh">
                    ...
                    {activeTab === 'analytics' && (
                        <div className="space-y-5">
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                                <StatCard label="Total Users" value={kpis.total} icon={Users} color="#3B82F6" />
                                <StatCard label="Premium Users" value={kpis.premium} icon={Crown} color="#F59E0B" />
                                <StatCard label="Free Users" value={stats.free} icon={Zap} color="#6B7280" />
                                <StatCard label="Conversion Rate" value={`${conversionRate}%`} icon={ArrowUpRight} color="#10B981" />
                            </div>
                            ...
                            <div className="bg-slate-900/40 border border-slate-800/80 backdrop-blur rounded-3xl p-6 space-y-4">
                                ...
                                <div className="space-y-3">
                                    {[
                                        { label: 'Free', count: stats.free, color: '#6B7280', total: kpis.total },
                                        { label: 'Silver', count: stats.silver, color: '#94A3B8', total: kpis.total },
                                        { label: 'Gold', count: stats.gold, color: '#F59E0B', total: kpis.total },
                                        { label: 'Diamond VIP', count: stats.diamond, color: '#A78BFA', total: kpis.total },
                            ...
                            <div className="bg-slate-900/40 border border-slate-800/80 backdrop-blur rounded-3xl p-6 space-y-4">
                                ...
                                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                                    <RevenueCard label="Today" value={stats.todayRevenue} color="emerald" />
                                    <RevenueCard label="This Week" value={stats.weekRevenue} color="sky" />
                                    <RevenueCard label="This Month" value={stats.monthRevenue} color="amber" />
                                    <RevenueCard label="All Time" value={stats.totalRevenue} color="rose" />
                                </div>
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md" onClick={() => setSelectedDocs(null)}>
                        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
                            ...
                            <div className="px-6 py-4 bg-slate-900/40 border-t border-slate-800 flex justify-end gap-3">
                                <button onClick={() => triggerAction(selectedDocs.userId, 'verify', { status: 'failed', reason: 'Documents do not match user information' })}
                                ...
                                <button onClick={() => triggerAction(selectedDocs.userId, 'verify', { status: 'verified' })}
