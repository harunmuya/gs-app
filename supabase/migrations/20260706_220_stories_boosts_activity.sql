...
begin
    if not exists (
        ...
        select 1
        from (
            select viewed_id, viewer_id, source, created_at, count(*) as row_count
            from public.profile_views
            group by viewed_id, viewer_id, source, created_at
            having count(*) > 1
        ) duplicate_rows
    ) then
...
begin
    if not exists (
        ...
        select 1
        from (
            select story_id, viewer_id, count(*) as row_count
            from public.story_views
            where viewer_id is not null
            group by story_id, viewer_id
            having count(*) > 1
        ) duplicate_rows
    ) then
...
begin
    if not exists (
        ...
        select 1
        from (
            select story_id, user_id, count(*) as row_count
