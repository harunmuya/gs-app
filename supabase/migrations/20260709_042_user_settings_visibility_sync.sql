-- User settings and Members visibility repair.
-- Safe for existing clients: no real user is deleted, banned users stay hidden.
-- Public visibility follows the Public Profile setting. The app can still warn incomplete users to complete their profile.
...
alter table public.user_settings
    ...
    add column if not exists live_location boolean not null default false,
...
create index if not exists users_members_visibility_idx on public.users (show_in_public, is_banned, is_suspended);
